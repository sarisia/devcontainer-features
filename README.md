...
| [`opencode`](#opencode) | `ghcr.io/sarisia/devcontainer-features/opencode:2` | npm | `~/.claude`, `~/.local/share/opencode`, `~/.agents` |
...
| [`codex`](#codex) | `ghcr.io/sarisia/devcontainer-features/codex:2` | npm | `~/.codex`, `~/.agents` |
...
Installs [OpenCode](https://opencode.ai) CLI and mounts `~/.claude` / `~/.local/share/opencode` / `~/.agents` from the host so settings and sessions persist.

```json
"features": {
    "ghcr.io/sarisia/devcontainer-features/opencode:2": {}
}
```
...
Installs [Codex CLI](https://developers.openai.com/codex/cli) via npm and mounts `~/.codex` / `~/.agents` from the host so auth and sessions persist. Requires Node.js — add the [node feature](https://github.com/devcontainers/features/tree/main/src/node) to your devcontainer.

```json
"features": {
    "ghcr.io/sarisia/devcontainer-features/codex:2": {}
}
```
